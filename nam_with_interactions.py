...
def train_nam(
    ...
    interaction_pairs: list = None,  # list of (feat_name_i, feat_name_j) from EBM
):
    """
    Train Neural Additive Model (NAM) with backfitting.

    NAM: y = bias + Σ_i f_i(x_i) [+ Σ_pq g_pq(x_p, x_q) if interaction_pairs given]

    Each f_i is a small MLP trained on the partial residual
    y - Σ_{j≠i} f_j(x_j) - Σ_pq g_pq(x_p, x_q).

    When interaction_pairs is provided (typically the 5 pairs chosen by EBM),
    a 2-input MLP g_pq is added for each pair and updated in the same
    backfitting loop, so single-feature and interaction networks learn
    complementary, non-redundant shape functions.
    ...
    * EBM-guided interaction pairs added as 2-input MLPs
    """
    ...
    # ── Resolve interaction pair column indices ───────────────────────────────
    pair_indices = []   # list of (col_i, col_j)
    pair_labels  = []   # list of "name_i x name_j" for printing
    if interaction_pairs:
        for name_i, name_j in interaction_pairs:
            if name_i in feat_name_to_idx and name_j in feat_name_to_idx:
                pair_indices.append((feat_name_to_idx[name_i], feat_name_to_idx[name_j]))
                pair_labels.append(f"{name_i} x {name_j}")
            else:
                missing = [n for n in (name_i, name_j) if n not in feat_name_to_idx]
                print(f"  [NAM] Skipping interaction pair ({name_i}, {name_j}) — "
                      f"feature(s) not in selected set: {missing}")
    ...
    # Interaction contributions (one column per pair)
    inter_contributions_train = np.zeros((X_train.shape[0], n_pairs))
    inter_contributions_test  = np.zeros((X_test.shape[0],  n_pairs))
    pair_models               = [None] * n_pairs

    train_losses, val_losses = [], []

    print(f"\nTraining NAM with {n_features} feature networks "
          f"+ {n_pairs} EBM-guided interaction pairs "
          f"({n_cycles} backfitting cycles)...")
    if pair_labels:
        print(f"  Interaction pairs: {pair_labels}")
    ...
    for cycle in range(n_cycles):
        ...
        for i in range(n_features):
            ...
            # Partial residual: y minus every other feature + all interaction contribs
            other_single  = contributions_train.sum(axis=1) - contributions_train[:, i]
            all_inter     = inter_contributions_train.sum(axis=1)
            partial_residual = y_train - other_single - all_inter
        ...
        # ── Interaction networks (one 2-input MLP per EBM pair) ───────────────
        for p_idx, (ci, cj) in enumerate(pair_indices):
            X_pair_train = X_train[:, [ci, cj]]
            X_pair_test  = X_test[:,  [ci, cj]]

            # Partial residual: y minus ALL single-feature contribs minus other interaction contribs
            all_single   = contributions_train.sum(axis=1)
            other_inter  = inter_contributions_train.sum(axis=1) - inter_contributions_train[:, p_idx]
            partial_residual = y_train - all_single - other_inter

            mlp_pair = MLPRegressor(
                hidden_layer_sizes=tuple(hidden_units),
                activation='relu',
                solver='adam',
                alpha=mlp_alpha,
                max_iter=epochs,
                random_state=random_state + n_features + p_idx + cycle * (n_features + n_pairs),
                early_stopping=True,
                validation_fraction=0.15,
                n_iter_no_change=20,
            )
            mlp_pair.fit(X_pair_train, partial_residual)

            inter_contributions_train[:, p_idx] = mlp_pair.predict(X_pair_train)
            inter_contributions_test[:,  p_idx] = mlp_pair.predict(X_pair_test)
            pair_models[p_idx] = mlp_pair

        if n_pairs > 0:
            print(f"    Trained {n_pairs} interaction networks.")

        # Track raw-sum RMSE at the end of each cycle (before Ridge)
        total_train = contributions_train.sum(axis=1) + inter_contributions_train.sum(axis=1)
        total_test  = contributions_test.sum(axis=1)  + inter_contributions_test.sum(axis=1)
        cycle_train_rmse = np.sqrt(mean_squared_error(y_train, total_train))
        cycle_val_rmse   = np.sqrt(mean_squared_error(y_test,  total_test))
    ...
    # Stack single-feature and interaction contributions side by side
    all_contributions_train = np.hstack([contributions_train, inter_contributions_train])
    all_contributions_test  = np.hstack([contributions_test,  inter_contributions_test])

    combiner = RidgeCV(alphas=[0.01, 0.1, 1.0, 10.0, 100.0])
    combiner.fit(all_contributions_train, y_train)
    print(f"\n  RidgeCV selected alpha = {combiner.alpha_:.4g}")

    y_pred_train = combiner.predict(all_contributions_train)
    y_pred_test  = combiner.predict(all_contributions_test)

    # Weighted single-feature contributions for interpretability plots
    contributions = [
        contributions_test[:, i:i+1] * combiner.coef_[i]
        for i in range(n_features)
    ]
    # Weighted interaction contributions (for optional heatmap plots)
    inter_contributions_weighted = [
        inter_contributions_test[:, p:p+1] * combiner.coef_[n_features + p]
        for p in range(n_pairs)
    ]

    _train_r2 = r2_score(y_train, y_pred_train)
    _test_r2  = r2_score(y_test,  y_pred_test)
    _k = n_features + n_pairs + 1
    _train_aic, _train_bic = compute_aic_bic(y_train, y_pred_train, _k)
    _test_aic,  _test_bic  = compute_aic_bic(y_test,  y_pred_test,  _k)
    metrics = {
        'train_rmse':    np.sqrt(mean_squared_error(y_train, y_pred_train)),
        ...
        'train_r2':      _train_r2,
        'test_r2':       _test_r2,
        'train_adj_r2':  adjusted_r2(_train_r2, len(y_train), n_features + n_pairs),
        'test_adj_r2':   adjusted_r2(_test_r2,  len(y_test),  n_features + n_pairs),
        'train_aic':     _train_aic,
        'test_aic':      _test_aic,
        'train_bic':     _train_bic,
        'test_bic':      _test_bic,
    ...
    class SklearnNAM:
        def __init__(self, feature_models, pair_models, pair_indices, combiner, n_features):
        ...
        def predict(self, X):
            contribs = np.zeros((X.shape[0], self.n_features + len(self.pair_models)))
            for i, mdl in enumerate(self.feature_models):
                contribs[:, i] = mdl.predict(X[:, i:i+1])
            for p, (mdl, (ci, cj)) in enumerate(zip(self.pair_models, self.pair_indices)):
                contribs[:, self.n_features + p] = mdl.predict(X[:, [ci, cj]])
            return self.combiner.predict(contribs)

    model = SklearnNAM(feature_models, pair_models, pair_indices, combiner, n_features)
    return model, metrics, y_pred_test, contributions, (train_losses, val_losses), inter_contributions_weighted, pair_labels
...
def train_ebm(X_train, X_test, y_train, y_test, feature_names, random_state: int = 42):
    ...
    # Get feature importances and print ALL terms clearly
    importances = ebm.term_importances()
    term_names = ebm.term_names_
    importance_order = np.argsort(importances)[::-1]

    print(f"\n  All EBM terms by importance:")
    interaction_pairs = []
    for rank, idx in enumerate(importance_order):
        tag = "  [interaction]" if ' x ' in term_names[idx] else ""
        print(f"    {rank+1:>2}. {term_names[idx]:<45s}  {importances[idx]:.4f}{tag}")
        if ' x ' in term_names[idx]:
            a, b = term_names[idx].split(' x ')
            interaction_pairs.append((a, b))

    print(f"\n  >>> EBM chose {len(interaction_pairs)} interaction pairs:")
...
def plot_ebm_interpretability(
    ...
    """Plot EBM main-effect shape functions (1-D) in original physical units.

    Interaction terms are skipped here — they get their own dedicated
    2-D heatmap figure from plot_ebm_interactions().
    """
    ebm_global = ebm.explain_global()
    importances = ebm.term_importances()
    term_names  = ebm.term_names_

    # Only main effects for this plot
    main_indices = [i for i, n in enumerate(term_names) if ' x ' not in n]
    main_indices_sorted = sorted(main_indices, key=lambda i: importances[i], reverse=True)

    n_plots = min(12, len(main_indices_sorted))
    n_cols  = 3
    ...
    for plot_i, idx in enumerate(main_indices_sorted[:n_plots]):
        ...
        if 'names' in data and 'scores' in data:
            ...
            if n > 0:
                ...
                ax.set_xticklabels([f'{x_orig[j]:.2g}' for j in tick_pos],
    ...
    plt.suptitle('EBM Main Effects — x in original feature units', fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"EBM main-effect plots saved to {save_path}")


def plot_ebm_interactions(
    ebm,
    save_path='ebm_interactions.png',
    scaler=None,
    all_feature_names=None,
):
    """Plot every EBM pairwise interaction term as a 2-D heatmap.

    Each panel shows the interaction score surface f_ij(x_i, x_j) on a grid
    in the original physical units of both features.  Red = high positive
    contribution to the prediction, blue = negative.

    Parameters
    ----------
    ebm               : fitted ExplainableBoostingRegressor
    save_path         : output PNG path
    scaler            : fitted scaler used during training (for inverse-transform)
    all_feature_names : full list of feature names the scaler was fitted on
    """
    ebm_global  = ebm.explain_global()
    importances = ebm.term_importances()
    term_names  = ebm.term_names_

    # Collect only interaction terms
    inter_indices = [i for i, n in enumerate(term_names) if ' x ' in n]
    if not inter_indices:
        print("  No interaction terms found in EBM.")
        return

    inter_indices_sorted = sorted(inter_indices, key=lambda i: importances[i], reverse=True)
    n_pairs = len(inter_indices_sorted)

    # Build inverse-transform helper
    name_to_full_idx = {}
    if scaler is not None and all_feature_names is not None:
        name_to_full_idx = {name: i for i, name in enumerate(all_feature_names)}
    ...
    print(f"\n  EBM interaction pairs ({n_pairs} total):")
    for plot_i, idx in enumerate(inter_indices_sorted):
        term_name  = term_names[idx]
        name_i, name_j = term_name.split(' x ')
        data       = ebm_global.data(idx)
        ax         = axes[plot_i]
        imp        = importances[idx]

        print(f"    {plot_i+1}. {term_name}  (importance={imp:.4f})")

        try:
            # interpret stores interaction data as:
            #   data['names'] = [bins_i, bins_j]   (each a list of bin edges)
            #   data['scores'] = 2-D array of shape (len(bins_i), len(bins_j))
            bins_i  = np.array(data['names'][0], dtype=float)
            bins_j  = np.array(data['names'][1], dtype=float)
            scores  = np.array(data['scores'])       # shape (len_i, len_j)

            # Inverse-transform bin centers to original units
            xi_orig = _inv_1d(bins_i, name_i)
            xj_orig = _inv_1d(bins_j, name_j)

            # pcolormesh needs shape (n_j, n_i) → transpose
            scores_T = scores.T
            vmax = np.abs(scores_T).max() or 1.0
            pcm = ax.pcolormesh(
                xi_orig, xj_orig, scores_T,
                cmap='RdBu_r', vmin=-vmax, vmax=vmax, shading='auto'
            )
            fig.colorbar(pcm, ax=ax, label='Interaction contribution')

            # Contour lines to show shape
            if scores_T.shape[0] > 1 and scores_T.shape[1] > 1:
                ax.contour(xi_orig, xj_orig, scores_T,
                           levels=6, colors='k', linewidths=0.5, alpha=0.4)

            ax.set_xlabel(f"{name_i} (original units)", fontsize=8)
            ax.set_ylabel(f"{name_j} (original units)", fontsize=8)
            ax.tick_params(axis='both', labelsize=7)
        ...
        ax.set_title(f"{term_name}\nimportance = {imp:.4f}", fontsize=9)

    for i in range(n_pairs, len(axes)):
        axes[i].set_visible(False)

    plt.suptitle('EBM Pairwise Interaction Surfaces\n'
                 '(red = pushes prediction up, blue = down)',
                 fontsize=13)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  EBM interaction heatmaps saved to {save_path}")
...
def main(filepath, seed: int = 42):
    ...
    # ── 12. Train EBM first — extract interaction pairs for NAM ─────────────────
    print("\n" + "="*70)
    print("[12] TRAINING EXPLAINABLE BOOSTING MACHINE (EBM)")
    print("="*70)
    ...
    try:
        ...
        # Extract the pairwise interaction terms EBM chose (e.g. "feat_a x feat_b")
        ebm_interaction_pairs = [
            tuple(name.split(' x '))
            for name in ebm_model.term_names_
            if ' x ' in name
        ]
        print(f"\n  EBM interaction pairs to graft into NAM ({len(ebm_interaction_pairs)} pairs):")
        for p in ebm_interaction_pairs:
            print(f"    {p[0]}  x  {p[1]}")
    ...
    # ── 13. Train NAM with EBM-guided interaction pairs ──────────────────────
    print("\n" + "="*70)
    print("[13] TRAINING NAM WITH EBM INTERACTIONS")
    print("="*70)

    (nam_model, nam_metrics, nam_pred, contributions,
     (train_losses, val_losses), inter_contributions, pair_labels) = train_nam(
        ...
        interaction_pairs=ebm_interaction_pairs if ebm_available else None,
    )
    ...
    # ── NAM interaction heatmaps ───────────────────────────────────────────────
    if inter_contributions and pair_labels:
        print("\n  Plotting NAM interaction heatmaps...")
        n_pairs_plot = len(pair_labels)
        fig_int, axes_int = plt.subplots(1, n_pairs_plot, figsize=(5 * n_pairs_plot, 4))
        if n_pairs_plot == 1:
            axes_int = [axes_int]
        for p_idx, (pair_name, contrib) in enumerate(zip(pair_labels, inter_contributions)):
            ax = axes_int[p_idx]
            name_i, name_j = pair_name.split(' x ')
            ci = list(feature_names_sel).index(name_i)
            cj = list(feature_names_sel).index(name_j)
            xi = X_test_sel[:, ci]
            xj = X_test_sel[:, cj]
            sc = ax.scatter(xi, xj, c=contrib.ravel(), cmap='RdBu_r', alpha=0.7, s=20)
            fig_int.colorbar(sc, ax=ax, label='Interaction contribution')
            ax.set_xlabel(name_i, fontsize=8)
            ax.set_ylabel(name_j, fontsize=8)
            ax.set_title(f'{pair_name}', fontsize=9)
        plt.suptitle('NAM Interaction Contributions (EBM-guided pairs)', fontsize=12)
        plt.tight_layout()
        plt.savefig('nam_interactions.png', dpi=150, bbox_inches='tight')
        plt.close()
        print("  NAM interaction heatmaps saved to nam_interactions.png")

    # EBM interpretability — main effects + interaction heatmaps
    if ebm_available:
        try:
            ...
            print(f"  Could not generate EBM main-effect plots: {e}")

        try:
            plot_ebm_interactions(
                ebm_model,
                save_path='ebm_interactions.png',
                scaler=scaler,
                all_feature_names=feature_names,
            ...
            print(f"  Could not generate EBM interaction plots: {e}")
    ...
    models_list = ['Random Forest', 'XGBoost', 'NAM+Interactions']
